...
class MockResponseFuture(Future):
    ...
    class MockSessionWithExecutor(object):
        ...
        def submit(self, fn, *args, **kwargs):
            # Fake submit... we are already in a thread due to add_callback mocking.
            fn(*args, **kwargs)
    ...
    def add_callback(self, fn, *args, **kwargs):
        # Important to run in background to not hang the main process due to how CQLEngineFuture works
        t = threading.Thread(target=fn, args=(self._result,)+args, kwargs=kwargs)
        t.start()
